...
export default function Dashboard() {
  ...
  // Mock data for chart if API doesn't provide historical data yet
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-card rounded-2xl border border-border p-6 shadow-sm flex flex-col">
            ...
            <h2 className="font-display font-bold text-xl mb-6">Conferências Recentes</h2>
            ...
            <div className="flex-1 overflow-auto pr-2 space-y-4">
              ...
              {conferences && conferences.length > 0 ? (
                ...
                conferences.slice(0, 5).map(conf => (
                  ...
                  <div key={conf.id} className="p-4 rounded-xl border border-border hover:bg-muted/50 transition-colors">

                    <div className="flex justify-between items-start mb-2">

                      <span className="font-bold text-foreground">Mapa {conf.mapNumber}</span>

                      <span className={`px-2 py-1 text-xs font-bold rounded-md ${

                        conf.status === 'completed' ? 'bg-green-100 text-green-700' :

                        conf.status === 'in_progress' ? 'bg-blue-100 text-blue-700' : 'bg-muted text-muted-foreground'

                      }`}>

                        {conf.status === 'completed' ? 'Finalizado' : conf.status === 'in_progress' ? 'Em andamento' : 'Pendente'}
