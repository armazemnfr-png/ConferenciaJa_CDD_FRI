...
const METRIC_COLORS = [
  ...
  { bg: "bg-orange-50",  border: "border-orange-400",  text: "text-orange-700",  sub: "text-orange-400",  label: "Checklist → Conferência", key: "checklistConferenceMin" as const },
  ...
  return (
    <AdminLayout>
      <div className="p-6 space-y-6">
        {/* Cabeçalho */}
        <div>
          <h1 className="text-2xl font-bold text-gray-900">TML — Tempo Médio de Liberação</h1>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right">CK→Conf</th>
                  <th className="px-4 py-3 text-right">Conferência</th>
                  <th className="px-4 py-3 text-right">Conf→Port</th>
                ...
                {isLoading ? (
                  ...
                  filtered.map((r, i) => (
                    <tr key={i} className="hover:bg-gray-50" data-testid={`tml-row-${i}`}>
                      ...
                      <td className="px-4 py-3 text-right font-mono text-amber-600">{r.matinalPatioMin > 0 ? minToHMS(r.matinalPatioMin) : "—"}</td>
                      <td className="px-4 py-3 text-right font-mono text-emerald-600">{r.checklistMin > 0 ? minToHMS(r.checklistMin) : "—"}</td>
                      <td className="px-4 py-3 text-right font-mono text-orange-600">{r.checklistConferenceMin > 0 ? minToHMS(r.checklistConferenceMin) : "—"}</td>
